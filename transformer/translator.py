...
class Translator(object):
    ...
    def translate_batch(self, src_batch):
        ...
        # Encode
        enc_outputs, _ =  self.model.encode(enc_inputs.to(device), enc_inputs_len.to(device))
        #enc_outputs, _ =  self.model.encode(enc_inputs, enc_inputs_len)

        # Repeat data for beam
        enc_inputs = (enc_inputs.repeat(1, beam_size).view(batch_size * beam_size, -1)).to(device)
        ...
        for i in range(self.opt.max_decode_step):
            ...
            for b in beams:
                if not b.done:
                    btmp = b.get_current_state()
                    #print(btmp)
                    dec_partial_inputs = torch.stack([btmp])
            ...
            if self.use_cuda:
                dec_partial_inputs = dec_partial_inputs.cuda().long()
                #print(dec_partial_inputs.long())
            ...
            def update_active_seq(seq_var, active_inst_idxs):
                ...
                with torch.no_grad():
                    v = Variable(active_seq_data)
                return v# Variable(active_seq_data, volatile=True)
            ...
            def update_active_enc_info(enc_info_var, active_inst_idxs):
                ...
                with torch.no_grad():
                    v = Variable(active_enc_info_data)
                return v# Variable(active_enc_info_data, volatile=True)
