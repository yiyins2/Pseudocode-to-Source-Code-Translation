...
def main(opt):
    ...
    for epoch in range(init_epoch + 1, opt.max_epochs + 1):
        ...
        torch.save(model_state, opt.model_path + "-ep" + str(epoch))
...
def train(model, criterion, optimizer, train_iter, model_state):  # TODO: fix opt
    ...
    for batch_idx, batch in enumerate(train_iter):
        ...
        dec_inputs = dec_[:, :-1].to(device)
        dec_targets = dec_[:, 1:].to(device)
        dec_inputs_len = (dec_inputs_len - 1).to(device)

        # Execute a single training step: forward
        optimizer.zero_grad()
        #print(enc_inputs.to(device).device)
        dec_logits, _, _, _ = model(enc_inputs.to(device), enc_inputs_len.to(device),
                                    dec_inputs.to(device), dec_inputs_len.to(device))
        step_loss = criterion(dec_logits.to(device), dec_targets.contiguous().view(-1))
        ...
        #print(step_loss.data.item())
        train_loss_total += float(step_loss.data.item()) if torch.cuda.is_available() else float(step_loss.data[0])
...
def eval(model, criterion, dev_iter):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(dev_iter):
            ...
            dec_targets = dec_[:, 1:].to(device)
            dec_inputs_len = (dec_inputs_len - 1).to(device)
            dec_logits, *_ = model(enc_inputs.to(device), enc_inputs_len.to(device), dec_inputs.to(device), dec_inputs_len.to(device))
            step_loss = criterion(dec_logits.to(device), (dec_targets.contiguous().view(-1)).to(device))
            eval_loss_total += float(step_loss.data.item()) if torch.cuda.is_available() else float(step_loss.data[0])
            n_words_total += torch.sum(dec_inputs_len)
